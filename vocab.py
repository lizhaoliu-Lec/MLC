...
def parse_args():
    parser = argparse.ArgumentParser(description='vocab.py for creating vocabulary')

    parser.add_argument('-train_src', default='./data/AAPD/text_train', type=str,
                        help="File of training source sentences")
    parser.add_argument('-train_tgt', default='./data/AAPD/label_train', type=str,
                        help="File of training target sentences (or label)")
    parser.add_argument('-size', default=5000, type=int,
                        help="vocab size")
    parser.add_argument('-freq_cutoff', default=2, type=int,
                        help="frequency cutoff")
    parser.add_argument('-vocab_file', default='./data/AAPD/vocab.json', type=str,
                        help="File of saving vocabulary")
...
class VocabEntry(object):
    ...
    def words2indices(self, sents):
        ...
        elif type(sents[0]) == str:
            return [self[w] for w in sents]
        else:
            raise ValueError('Expect sents of type list[str] or list[list[str]], but got type of %s[%s]' % (
                type(sents), type(sents[0])))
...
if __name__ == '__main__':
    ...
    print('read in target sentences (labels): %s' % args.train_tgt)

    src_sents = read_corpus(args.train_src, source='src')
    tgt_sents = read_corpus(args.train_tgt, source='tgt')

    vocab = Vocab.build(src_sents, tgt_sents, args.size, args.freq_cutoff)
